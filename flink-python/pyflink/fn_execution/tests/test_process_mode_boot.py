...
class PythonBootTests(PyFlinkTestCase):
    ...
    def test_set_working_directory(self):
        ...
        os.mkdir(mock_pyflink_dir)
        # just create an empty file
        open(os.path.join(mock_pyflink_dir, "__init__.py"), 'a').close()
        mock_fn_execution_dir = os.path.join(mock_pyflink_dir, "fn_execution")
        os.mkdir(mock_fn_execution_dir)
        open(os.path.join(mock_fn_execution_dir, "__init__.py"), 'a').close()
        with open(os.path.join(mock_fn_execution_dir, "boot.py"), "w") as f:
