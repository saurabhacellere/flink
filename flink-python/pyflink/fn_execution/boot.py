...
def pip_install_requirements_process_mode():
    if (PYTHON_REQUIREMENTS_FILE in os.environ
        ...
        if "PATH" in os.environ:
            os.environ["PATH"] = \
                os.pathsep.join(
                    [os.path.join(requirements_install_path, "bin"), os.environ["PATH"]])
        else:
            os.environ["PATH"] = os.path.join(requirements_install_path, "bin")
        env = dict(os.environ)
        from distutils.dist import Distribution
        install_obj = Distribution().get_command_obj('install', create=True)
        install_obj.prefix = requirements_install_path
            ...
            while True:
                ...
                if text_line:
                    ...
                    if parse_version(pip_version) >= parse_version('8.0.0'):
                        py_args.extend(
                            ["--prefix", requirements_install_path, "--ignore-installed"])
                    else:
                        py_args.extend(['--install-option', '--prefix=' + requirements_install_path,
                                        "--ignore-installed"])
                    if requirements_cache_path is not None:
                        py_args.extend(["--find-links", requirements_cache_path])
