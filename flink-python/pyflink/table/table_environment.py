...
class TableEnvironment(object):
    ...
    def add_python_file(self, file_path):
        """
        Upload single python file, python package or local directory to cluster.
        These files and directory will be appended to the PYTHONPATH of the python UDF worker.
        Please make sure they can be imported directly.

        This method can be used to upload the dependencies of python UDFs to the cluster.

        :param file_path: The path of python file, python package or local directory.
        :type file_path: str
        """
    ...
    def set_python_requirements(self, requirements_file_path, requirements_cache_dir=None):
        """
        Upload a "requirements.txt" file to specify the third-party libraries used in python UDF.
        These libraries will be installed to a temporary directory and be imported to the python
        UDF worker.

        This method can be used to specify the third-party dependencies via requirements.txt.

        If the cluster is running on a private network that cannot access the PyPI central
        repository, a local directory which contains all the packages listed in the
        "requirements.txt" file should be specified using the "requirements_cached_dir" parameter.
        It will be uploaded to cluster to support offline installation.
        ...
        .. note::
            ...
            Please make sure the package file matches the platform and python version of cluster.
        ...
        :param requirements_cache_dir: The path of the local directory contains all packages
    ...
    def add_python_archive(self, archive_path, target_dir=None):
        """
        Upload a packaged file to cluster. The file will be extracted to the working directory of
        python UDF worker.

        If parameter "target_dir" is specified, the archive file will be extracted to a directory
        named ${target_dir}. Otherwise, the archive file will be extracted to a directory with the
        same name of the archive file.

        If python UDFs need to run in a specific python environment and the environment does not
        exist on cluster, this method can be used to upload the packaged environment.
        Note that the python interpreter path of the uploaded environment should be specified
        via the method :func:`pyflink.table.TableConfig.set_python_executable`.

        The files uploaded via this method are also accessible in UDFs. Using relative path to
        access them.
        ...
        .. note::
            ...
            running on and that the version is 3.5 or higher.

        .. note::

            Currently only zip-format is supported. i.e. zip, jar, whl, egg and other zip-format
            files are all supported, but tar, tar.gz, 7z, rar and other archive format files are not
            supported.
