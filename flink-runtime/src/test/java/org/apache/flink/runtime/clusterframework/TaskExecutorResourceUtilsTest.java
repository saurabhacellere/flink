...
public class TaskExecutorResourceUtilsTest extends TestLogger {
	...
	public void testConfigDefaultSlotFraction() {
		...
		Configuration conf = new Configuration();
		conf.setFloat(TaskManagerOptions.DEFAULT_SLOT_FRACTION, (float) defaultSlotFraction);
		...
		validateInAllConfigurations(conf, taskExecutorResourceSpec -> {
			...
			assertThat(defaultSlotResourceProfile.getCpuCores(),
				is(taskExecutorResourceSpec.getCpuCores().isPresent() ? taskExecutorResourceSpec.getCpuCores().get().multiply(defaultSlotFraction) : CPUResource.MAX_VALUE));
	...
	public void testConfigDefaultSlotFractionLegacyNumSlots() {
		...
		validateInAllConfigurations(conf, taskExecutorResourceSpec -> {
			...
			final ResourceProfile defaultSlotResourceProfile = TaskExecutorResourceUtils.generateDefaultSlotResourceProfile(taskExecutorResourceSpec);
			assertThat(defaultSlotResourceProfile.getCpuCores(),
				is(taskExecutorResourceSpec.getCpuCores().isPresent() ? taskExecutorResourceSpec.getCpuCores().get().multiply(fraction) : CPUResource.MAX_VALUE));
	...
	@Test
	public void testConfigDefaultSlotFractionFailure() {
