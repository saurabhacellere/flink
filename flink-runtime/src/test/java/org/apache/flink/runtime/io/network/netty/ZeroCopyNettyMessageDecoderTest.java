...
public class ZeroCopyNettyMessageDecoderTest {
    ...
    public void testDownstreamMessageDecode() throws Exception {
        ...
        EmbeddedChannel channel = new EmbeddedChannel(
                new ZeroCopyNettyMessageDecoder(
                        new BufferResponseAndNoDataBufferMessageParser(
                                new NetworkBufferAllocator(
                                        createPartitionRequestClientHandler(8)))));
        NettyMessage.BufferHolder<Buffer> event = createDataBuffer(32);
        event.getBuffer().tagAsEvent();

        NettyMessage[] messages = new NettyMessage[]{
                new NettyMessage.BufferResponse<>(createDataBuffer(128), true, 0, NORMAL_INPUT_CHANNEL_ID, 4),
                new NettyMessage.BufferResponse<>(createDataBuffer(256), true, 1, NORMAL_INPUT_CHANNEL_ID, 3),
                new NettyMessage.BufferResponse<>(event, false, 2, NORMAL_INPUT_CHANNEL_ID, 4),
                new NettyMessage.ErrorResponse(new RuntimeException("test"), NORMAL_INPUT_CHANNEL_ID),
                new NettyMessage.BufferResponse<>(createDataBuffer(56), true, 3, NORMAL_INPUT_CHANNEL_ID, 4)
        };
    ...
    public void testDownstreamMessageDecodeWithReleasedInputChannel() throws Exception {
        ...
        NettyMessage[] messages = new NettyMessage[]{
                new NettyMessage.BufferResponse<>(createDataBuffer(128), true, 0, NORMAL_INPUT_CHANNEL_ID, 4),
                new NettyMessage.BufferResponse<>(createDataBuffer(256), true, 1, RELEASED_INPUT_CHANNEL_ID, 3),
                new NettyMessage.BufferResponse<>(event, false, 2, NORMAL_INPUT_CHANNEL_ID, 4),
                new NettyMessage.ErrorResponse(new RuntimeException("test"), RELEASED_INPUT_CHANNEL_ID),
                new NettyMessage.BufferResponse<>(createDataBuffer(64), false,3, NORMAL_INPUT_CHANNEL_ID, 4),
        };
    ...
    private void readInputAndVerify(EmbeddedChannel channel, ByteBuf[] inputBuffers, NettyMessage[] expected) throws Exception {
        ...
        for (int i = 0; i < expected.length; ++i) {
            ...
            if (expected[i] instanceof NettyMessage.AddCredit ||
                ...
                assertEquals(((NettyMessage.BufferResponse) expected[i]).getBufferSize(), ((NettyMessage.BufferResponse) decodedMessages.get(i)).getBufferSize());
    ...
    private NettyMessage.BufferHolder<Buffer> createDataBuffer(int size) {
        ...
        return new NettyMessage.FlinkBufferHolder(buffer);
    }
    ...
    private CreditBasedPartitionRequestClientHandler createPartitionRequestClientHandler(int numberOfBuffersInNormalChannel) throws IOException {
        ...
        SingleInputGate singleInputGate = createSingleInputGate(2);
        ...
        RemoteInputChannel normalInputChannel = spy(new RemoteInputChannel(
                ...
                0,
                0,
                mock(InputChannelMetrics.class),
                networkBufferPool));
        when(normalInputChannel.getInputChannelId()).thenReturn(NORMAL_INPUT_CHANNEL_ID);
        singleInputGate.setInputChannel(normalInputChannel.getPartitionId().getPartitionId(), normalInputChannel);
        ...
        singleInputGate.setInputChannel(releasedInputChannel.getPartitionId().getPartitionId(), releasedInputChannel);
